...
## B16-B18 - Competitive wave: queues, tags, insight-at-decision (spec'd + BUILT 2026-07-18)

From a Hootsuite/Sprout Social gap analysis. Full spec (DB, endpoints, tests, skip list,
parking lot): `docs/B16_B18_COMPETITIVE_WAVE_SPEC.md`. Shipped same day, suite 219 -> 247.

- **B16a Queue slots**: `queue_slots` (mig v8) + `src/queue.js` `nextOpenSlot` (weekly walk,
  collision/quiet-hours/past skip) + `/api/queue-slots` CRUD + `POST /api/posts/:id/queue`.
  Settings per-brand Queues editor + seed button; composer "Add to queue".
- **B16b Nav rail**: grouped left nav (Plan/Create/Grow/Setup), collapsible w/ localStorage
  state, icon-only under 900px.
- **B17a Tags & campaigns**: `tags` + `post_tags` (mig v9) + `src/tags.js` (max one campaign
  per post), tags in all post payloads, `?tag_id=` scoped analytics; composer picker,
  calendar campaign-colored chips + tag filter, Analytics campaign selector.
- **B17b Gap-finding**: month per-platform count dots + future-empty-day treatment, week
  header counts, brand coverage strip (zero-coverage warning -> composer).
- **B18a Best-time**: `src/besttime.js` + `GET /api/best-times` (own-metrics bucketing >= 8
  published posts, else static `best_times` defaults in platform-specs); composer schedule
  nudge with click-to-apply chips + queue-editor hint.
- **B18b Redraft the winner**: Analytics top-10 rows -> composer with original staged as
  grounding/example, auto-runs Draft with AI ("fresh take, new hook").
- **B18c UTM auto-append**: `src/utm.js`, per-brand utm_enabled/utm_template (settings
  `utm:{brand_id}`), applied once at the approve gate, idempotent, `{campaign}` = post's
  campaign tag else brand slug.

## D2 - Design consistency pass (Seeds-informed) (spec'd + BUILT 2026-07-18)

Full spec: `docs/D2_CONSISTENCY_PASS_SPEC.md`; audit: `docs/D2_AUDIT.md`. Sprout's
discipline, PostDeck's identity (ink/gold kept).

- Primitives: `pageHeader/formSection/toast/inlineBanner/emptyState` helpers; button system
  (sm 28 / md 36 / lg 44 x primary/secondary/ghost/destructive, full interaction states,
  dead `.btn-secondary` defined); matched input tiers; `.switch` toggles; width tiers
  (`view-default` 1400 / `view-narrow` 736 / `view-flush`); global focus-visible ring;
  24px touch-target floor.
- Composer reordered to Sprout flow: distribution -> content -> media -> metadata ->
  scheduling -> AI tools (one collapsible); alert() -> toast; platform-tab vs primary-CTA
  gold collision fixed.
- Settings zoned: Workspace / Brands / Integrations & Ops with anchor nav.
- Sweep of all other views: R1 header order (calendar toolbar: nav left, filters rightmost),
